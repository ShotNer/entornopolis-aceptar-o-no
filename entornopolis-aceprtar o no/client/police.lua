...
AddEventHandler('enp-entorno:forzar:sendNotify', function( modelo, primary, matricula, location )
  if Config.Notify == true  then 
    ...
    TriggerEvent("kiri_notifications:SetQueueMax", "forzar", 5)
    TriggerEvent('kiri_notifications:SendNotification',{text = "Robo de Vehiculo  ~n~ Modelo: "..modelo.."  ~n~ Color: "..primary.."~n~ Matricula: "..matricula.." ~n~ Ubicación: "..location.." ~n~ ~g~[E Aceptar]~w~ ~r~[Q Denegar]", 
      timeout = 15000,
      type = "info",
      progressBar = true,
      layout     = "centerRight",
...
AddEventHandler('enp-entorno:entorno:sendNotify', function( msg, location )
  if Config.Notify == true  then 
    if job == 'police' then 
      TriggerEvent("pNotify:SetQueueMax", "forzar", 5)
      TriggerEvent('pNotify:SendNotification',{text = "Entono  </br> "..msg.."  en  "..location.." </br> <p> </p><b style='color:#3efe00'>[E Aceptar] </b> <b style='color:#fe0000'>[Q Denegar] </b>", 
      timeout = 15000,
        type = "info",
        progressBar = true,
        layout     = "centerRight"
    ...
    TriggerEvent("kiri_notifications:SetQueueMax", "forzar", 5)
    TriggerEvent('kiri_notifications:SendNotification',{text = "Entono  ~n~ "..msg.."  en  "..location.." ~n~ ~g~[E Aceptar] ~r~[Q Denegar] ", 
      timeout = 15000,
      type = "info",
      progressBar = true,
      layout     = "centerRight"
